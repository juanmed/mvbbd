...
void BBox::setNumPoints(int num_points)
{
    Points = Eigen::MatrixXd::Zero ( num_points, 3 );
    T = Eigen::Matrix<double, 4, 4>::Identity();
}
...
void BBox::SetPoints ( const Eigen::MatrixXd& EigenPoints )
...
void BBox::SetT ( Eigen::Matrix<double , 4, 4>& Tnew )
{
    Eigen::Matrix<double, 4, 4> Tprev = T;
    T = T*Tnew;
}

void BBox::SetPoint ( int i, double x, double y, double z )
{
...
void BBox::doPCA ( const Eigen::Matrix<double, 4, 4>& Told )
{
...
void  BBox::box_distance ( BBox bigestbox, BBox actual)
{ 
