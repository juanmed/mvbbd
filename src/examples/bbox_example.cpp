...
int main ( int argc, char* argv[] )
{
    ...
    BBox ObjectOriginal ( num_points );
    ...
    std::list<BBox> sorted_boxes;
    sorted_boxes = extractBoxes ( ObjectOriginal , gain, min_volume);

    std::vector<std::pair<BBox, std::vector< Eigen::MatrixXd> > > box_and_transformations;
    ...
    while (!sorted_boxes.empty())
    {
        if (method != 0 )
        {
            // box_and_transformations.push_back( std::pair<BBox, std::vector< Eigen::MatrixXd> > (sorted_boxes.front(), get_populated_TrasformsforHand( sorted_boxes.front(), ObjectOriginal)));
        }
        else
        {
            std::vector<Eigen::MatrixXd> transformation;
            transformation.push_back(info_adams(sorted_boxes.front(), ObjectOriginal));
            box_and_transformations.push_back( std::pair<BBox, std::vector< Eigen::MatrixXd> > (sorted_boxes.front(), transformation));
        }
