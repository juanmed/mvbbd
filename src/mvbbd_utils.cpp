...
std::vector<BBox> FindBestSplit ( BBox Object_in, double gain )
{
    ...
    if ( cutting_point_vec.size() > 0 )
    {
        ...
        BBox Box1 ( temp_object1.size() );
        BBox Box2 ( temp_object2.size() );
...
Eigen::MatrixXd info_adams( BBox  first_boxes, BBox ObjectOriginal, double distance, bool mirrorred)
{
...
std::list<BBox> extractBoxes ( BBox ObjectOriginal, float gain, float min_volume, float min_points )
{


    std::list<BBox> results;
    std::vector< BBox > SplitedObject;
    std::list< BBox > cue;
...
std::vector<Eigen::MatrixXd> getTransformsForHand(std::list<BBox> sorted_boxes, BBox ObjectOriginal, double dist_hand)
{
...
std::vector<Eigen::MatrixXd> get_populated_TrasformsforHand(BBox box, BBox ObjectOriginal, int disc, double dist_hand)
{
    std::vector<Eigen::MatrixXd> results;
...
std::vector<Eigen::MatrixXd> filter_poses(std::vector<Eigen::MatrixXd> all_poses, BBox ObjectOriginal)
{
    std::vector<Eigen::MatrixXd> results;
